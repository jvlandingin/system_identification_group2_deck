---
title: "System Identification: Selected Topics"
subtitle: "Chapter 8 (Section 8.5) and Chapter 10 (Sections 10.1, 10.2, 10.5)"
...
**Part 1: Chapter 8 - Convergence and Consistency** - Section 8.5: Frequency-Domain Description of the Limit Model

. . .

**Part 2: Chapter 10 - Computing the Estimate** - Section 10.1: Linear Regressions and Least Squares - Section 10.2: Numerical Solution by Iterative Search Methods - Section 10.5: Local Solutions and Initial Values
...
**Solution**: Use iterative numerical techniques to find the minimum/solution.
...
**Key property**: Gives **one-step convergence** for quadratic functions!

. . .

**Practical issue**: Computing the full Hessian $V''(\theta)$ can be expensive.
...
This formula comes from approximating $V(\theta)$ by a quadratic function around the current point $\hat{\theta}^{(i)}$ and finding the minimum of that quadratic approximation.
...
-   **One-step for quadratics**: If $V(\theta)$ is exactly quadratic, Newton finds the minimum in one step
...
Computing all $d^2$ entries of the Hessian and then inverting it costs $O(d^3)$ operations. For large parameter spaces, this is prohibitively expensive. This motivates quasi-Newton methods.
:::
...
where $\psi(t, \theta)$ is the $d \times p$ gradient matrix of $\hat{y}(t|\theta)$ with respect to $\theta$.

. . .

**Major computational burden**: Computing $\psi(t, \theta)$ for $t = 1, 2, \ldots, N$

::: notes
**Understanding the gradient formula:**

Say "V N prime of theta, Z to the N equals negative one over N times the sum..."

This formula tells us how to compute the direction of steepest ascent of the criterion function. The negative of this gives us the steepest descent direction.

**Components of the formula:**

-   $\psi(t, \theta)$: The gradient of the predictor $\hat{y}(t|\theta)$ with respect to parameters. This is a matrix because the output might be multidimensional.

-   $\ell_{\varepsilon}'(\varepsilon(t, \theta), \theta)$: Derivative of loss function with respect to the prediction error

-   $\ell_{\theta}'(\varepsilon(t, \theta), \theta)$: Direct derivative of loss function with respect to parameters (often zero)

**Why computing** $\psi(t, \theta)$ is hard:

For each time point $t = 1, 2, \ldots, N$, we need to compute how the predicted output changes with respect to each parameter. This requires:

-   Understanding the model structure
-   Chain rule for composite functions
-   Potentially recursive calculations for dynamic models

Section 10.3 discusses efficient techniques for computing $\psi(t, \theta)$ for common model structures.

**Analogy:**

Think of the gradient as a compass that points uphill. At each iteration, we compute this compass direction and then walk downhill (negative gradient) to decrease $V(\theta)$.
:::

------------------------------------------------------------------------

## Scalar Output and Quadratic Criterion

Consider the special case:

$$V_N(\theta, Z^N) = \frac{1}{N}\sum_{t=1}^{N} \frac{1}{2}\varepsilon^2(t, \theta)$$

. . .

This is the **nonlinear least-squares problem**.

. . .

The gradient simplifies to:

$$V_N'(\theta, Z^N) = -\frac{1}{N}\sum_{t=1}^{N} \psi(t, \theta)\varepsilon(t, \theta)$$

. . .

A general family of search algorithms:
...
This is one of the most important special cases in system identification. We're minimizing the sum of squared prediction errors, just like linear regression, but now the prediction errors $\varepsilon(t, \theta)$ are non-linear functions of $\theta$.

**Why this case matters:**

-   Most common criterion in practice (quadratic loss)
-   Well-studied numerically (extensive literature)
-   Good balance of statistical properties and computational tractability

**The general search formula:**

Say "theta hat N at iteration i minus 1 equals theta hat N at iteration i minus mu N at iteration i times R N at iteration i inverse times V N prime..."

This is a template that covers many specific algorithms:

-   $R_N^{(i)}$: A matrix that modifies the search direction (we'll discuss choices next)
-   $\mu_N^{(i)}$: Step size parameter (will be discussed)
-   $V_N'(\hat{\theta}_N^{(i)}, Z^N)$: The gradient at the current iterate

**The negative sign:**

Note the minus sign in the formula - we're moving in the direction opposite to the gradient, which is downhill.

**What to emphasize:**

This formula is the foundation for the next several slides where we'll explore different choices for $R_N^{(i)}$ and $\mu_N^{(i)}$ that give rise to different algorithms.
:::
...
**Pros**: Simple, doesn't require second derivatives

**Cons**: Fairly inefficient close to the minimum - convergence can be very slow
...
The Hessian for the quadratic criterion is:

$$V_N''(\theta, Z^N) = \frac{1}{N}\sum_{t=1}^{N} \psi(t, \theta)\psi^T(t, \theta) - \frac{1}{N}\sum_{t=1}^{N} \psi'(t, \theta)\varepsilon(t, \theta)$$

where $\psi'(t, \theta)$ is the $d \times d$ Hessian of $\varepsilon(t, \theta)$.

. . .

**Key approximation**: Near the minimum, errors $\varepsilon(t, \theta_0) = e_0(t)$ are independent with $Ee_0(t) = 0$

. . .

This makes the second sum negligible, giving:

$$V_N''(\theta, Z^N) \approx \frac{1}{N}\sum_{t=1}^{N} \psi(t, \theta)\psi^T(t, \theta) \triangleq H_N(\theta)$$
...
Choosing $R_N^{(i)} = H_N(\hat{\theta}_N^{(i)})$ gives:
...
-   Only requires **first derivatives** (no Hessian computation!)
-   $H_N(\theta)$ is always **positive semidefinite** → descent algorithm guaranteed
-   Good convergence near minimum

. . .

**Common choice**: $\mu_N^{(i)} = 1$ (called the "method of scoring" in statistics)
...
**Problem**: $H_N(\theta)$ may be singular or nearly singular (especially if model is overparameterized).

. . .

**Solution**: Use a **regularized** Hessian approximation:

$$R_N^{(i)}(\lambda) = \frac{1}{N}\sum_{t=1}^{N} \psi(t, \hat{\theta}_N^{(i)})\psi^T(t, \hat{\theta}_N^{(i)}) + \lambda I$$

. . .

-   $\lambda > 0$ is a **regularization parameter**
-   $\mu_N^{(i)} = 1$ (fixed step size)
-   $\lambda$ controls convergence, not step size

. . .

**Effect of** $\lambda$:

-   $\lambda = 0$: Standard Gauss-Newton
-   $\lambda$ large: Smaller steps toward gradient direction (like steepest descent)
...
**Aspect 1: Limit criterion** $\bar{V}(\theta)$

-   As $N \to \infty$, the limit $\bar{V}(\theta) = \lim_{N \to \infty} V_N(\theta, Z^N)$ may have local minima
-   Then $V_N(\theta, Z^N)$ will have such minima for large $N$ (Lemma 8.2)

. . .

**Aspect 2: Finite-sample randomness**

-   Even if $\bar{V}(\theta)$ has only one local minimum (= global), $V_N(\theta, Z^N)$ may have other local minima due to randomness in data
-   Much harder to analyze analytically

. . .

**Exception**: Linear regression LS has no false local minima by construction

::: notes
**Understanding the two sources of local minima:**

**Aspect 1 - Structural local minima:**

Say "V bar of theta" for the limit criterion.

These local minima are inherent to the problem formulation:

-   $\bar{V}(\theta)$ is the expected value of the criterion as the amount of data goes to infinity
-   If $\bar{V}(\theta)$ itself has multiple local minima, then for large enough $N$, $V_N(\theta)$ will have minima near those same locations
-   This is a property of the **model structure** and the **true system**, not just sampling variation

**Why this happens:**

-   Complex model structures (e.g., neural networks, nonlinear models)
-   Model mismatch (true system not in model set)
-   Identifiability issues (different parameter values give similar predictions)

**Aspect 2 - Sample-induced local minima:**

Even if the population criterion $\bar{V}(\theta)$ is convex (single minimum), the finite-sample criterion $V_N(\theta, Z^N)$ can have multiple local minima due to:

-   Random fluctuations in the data
-   Outliers
-   Finite sample effects

This is much harder to characterize theoretically because it depends on the specific data realization.
...
-   **ARARX models** ($C = F \equiv 1$): No false minima if SNR is large enough
...
**The crucial assumption** $S \in \mathcal{M}$:

Say "S in script M" - the true system is in the model set.

These theoretical results assume the model structure is correct - the true system can be exactly represented by some parameter value in your model class. This is a strong assumption!
...
-   Use physical insight for reasonable initial values
...
**The problem**: Filters require initial values $\varphi(0, \theta)$

Recall the predictor filter:

$$\hat{y}(t|\theta) = H_f(q, \theta)y(t) + H_g(q, \theta)u(t)$$

which requires past values to start.

. . .

**Four approaches to handle initial conditions:**

1.  **Set to zero**: $\varphi(0, \theta) = 0$
2.  **Match data exactly**: Choose $\varphi(0, \theta)$ so first $\hat{y}(t|\theta), t = 1, \ldots, \dim\varphi$ match $y(t)$ exactly
3.  **Treat as parameter**: Estimate $\varphi(0, \theta) = \eta$ along with $\theta$
4.  **Backforecast**: Estimate $\varphi(0, \theta)$ from data by running filters backwards (Knudsen, 1994)

::: notes
**Why we need initial conditions:**

The predictor filters (equations 10.53-10.54 and 10.56) are recursive:

$$\varphi(t|\theta) = F(\varphi(t-1|\theta), y(t-1), u(t-1), \theta)$$

To start the recursion at $t = 1$, we need $\varphi(0, \theta)$ - the filter state before we have any data.

**Special case - Finite impulse response:**

For ARX models with $n_a = 0$ (FIR), the filters have finite impulse response. In this case, we can wait to initialize until enough past data are known. This is "approach 1 following (10.13)" from Section 10.1.

**General case:**

For models with infinite impulse response (IIR), we need a strategy for the unknown initial conditions.

**Approach 1: Set to zero**

$\varphi(0, \theta) = 0$

-   **Pros**: Simple, always works
-   **Cons**: Can cause transient errors, especially if system has slow dynamics
-   **When to use**: When the filter transient is short compared to the data length

**Approach 2: Match data exactly**

Choose $\varphi(0, \theta)$ so that the first few predictions $\hat{y}(t|\theta)$ match $y(t)$ exactly.

-   **Pros**: No initial transient error
-   **Cons**: Requires solving for initial conditions, may not always be possible
-   **When to use**: When you want to avoid any initial transient

**Approach 3: Treat as parameters**

Introduce $\eta = \varphi(0, \theta)$ as additional parameters and estimate them along with $\theta$.

-   **Pros**: Optimal in some sense, treats initial conditions probabilistically
-   **Cons**: Increases parameter dimension, more complex optimization
-   **When to use**: When initial conditions are truly uncertain and important

**Approach 4: Backforecast**

Run suitable filters backwards in time from the data to estimate $\varphi(0, \theta)$ (Knudsen, 1994).

-   **Pros**: Uses all available data, no arbitrary choices
-   **Cons**: More complex implementation, requires additional computation
-   **When to use**: For sophisticated applications where initial conditions matter

**When does it matter?**

For models where the predictor filter **transient is short** compared to the data record, it doesn't matter much which approach is taken.

However, with **slowly decaying transients**, the first two methods may have a **very negative influence** on model quality. This is particularly pronounced for **OE models** (Output Error), where no noise model will pick up the residuals from bad transient behavior.

**What to emphasize:**

For most practical applications with reasonable amounts of data, approach 1 (zero initialization) is sufficient. Only worry about more sophisticated approaches when dealing with slow dynamics or short data records.
:::
