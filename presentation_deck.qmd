...
Your regression vector would look like:
$$\varphi(t) = \begin{bmatrix} -y(t-1) \\ -y(t-2) \\ u(t-1) \\ u(t-2) \end{bmatrix}$$

Where:
- $y(t-1), y(t-2)$ are the **past 2 outputs** (the autoregressive part)
- $u(t-1), u(t-2)$ are the **past 2 inputs** (the exogenous part)
...
When you use prewindowing, $R(N)$ becomes a **block Toeplitz matrix**, meaning:
...
The **Toeplitz structure** from Approach 2 enables a fast recursive algorithm:
...
where $\rho_n = -\alpha_n / V_n$ is a **reflection coefficient**

. . .

**Computational advantage:**

-   Going from order $n$ to $n+1$: Only $O(n)$ operations
-   Computing all orders 1 to $n$: Total $O(n^2)$ operations
-   Compare to QR: $O(n^3)$ for solving once
-   **Developed by Levinson (1947)** — widely used in signal processing

------------------------------------------------------------------------

## Lattice Filters (*)

An alternative recursive structure that implements the same estimation as Levinson, but with a different form:

**Architecture:**

-   **Forward error**: $\hat{e}_n(t)$ — prediction error of order $n$
-   **Backward error**: $\hat{f}_n(t-1)$ — backward prediction error
-   **Orthogonality**: Errors at different orders are mutually orthogonal!

$$\frac{1}{N}\sum_{t=1}^{N} \hat{e}_n(t)\hat{e}_{n-k}(t-k) = \begin{cases} V_n, & k = 0 \\ 0, & k \neq 0 \end{cases}$$

. . .

**Key parameters: Reflection coefficients** $\rho_n$ (or PACF)

-   **Bounded**: Always $|\rho_n| < 1$ for stable systems
-   **Interpretation**: Represent structural information (e.g., acoustic reflections in speech)
-   **Stability indicator**: If $|\rho_n| \approx 1$ → system nearly unstable

. . .

**Why lattice filters are superior:**

-   **Numerical stability**: Bounded reflection coefficients preserve orthogonality
-   **Recursive QR**: Processes data one time step at a time (adaptive/real-time)
-   **Applications**: Speech processing, Kalman filtering, adaptive systems
