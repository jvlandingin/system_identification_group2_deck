...
::: notes
Let me give you a concrete example to make this tangible.

**For an ARX model** with n_a = n_b = n, z(t) is a vector containing the negative output and the input. So you're stacking both output history and input history into your regressor.

[→ Click for AR model]

**For an AR model**—a pure autoregressive process—z(t) is just the negative output. No input terms, just past outputs predicting the current output.
:::
