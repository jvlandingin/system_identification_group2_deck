...
**Examples where analytical solutions fail:**

- Non-linear model structures (Hammerstein-Wiener models)
- Non-quadratic loss functions (robust estimation)
- Output error models (prediction errors non-linear in parameters)
...
**The Hessian matrix** for parameter vector of dimension $d$:
...
This is the **nonlinear least-squares problem** - most common in practice!

. . .

**Why this case matters:**

- Quadratic loss → good statistical properties
- Well-studied numerically
- Balance of accuracy and computational tractability
...
**General family of search algorithms:**

$$\hat{\theta}_N^{(i+1)} = \hat{\theta}_N^{(i)} - \mu_N^{(i)} [R_N^{(i)}]^{-1} V_N'(\hat{\theta}_N^{(i)}, Z^N)$$

Different choices of $R_N^{(i)}$ and $\mu_N^{(i)}$ give different algorithms
